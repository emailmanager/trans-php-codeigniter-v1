...
/**
 * Emailmanager Email Library
 *
 * Permits email to be sent using emailmanager.com's Servers
 *
 * @category	Libraries
...
class Emailmanager {
     ...
     */	
    function Emailmanager($params = array())
    {
    	...
    	{
    	   $this->api_key = 'EMAILMANAGER_API_TEST';
    	}
    	
        log_message('debug', 'Emailmanager Class Initialized');
    ...
    */
    ...
    function send($from_address = null, $from_name = null, $to_address = null, $to_name = null, $subject = null, $message_plain = null, $message_html = null)
	{
        ...
        if (!function_exists('curl_init'))
        {
            ...
            {
                log_message('error', 'Emailmanager - PHP was not built with cURL enabled. Rebuild PHP with --with-curl to use cURL.');            
            }
		...
		if (is_null($this->api_key)) {
			show_error("Emailmanager API key is not set!");
		}
		...
		$return = curl_exec($ch);
		log_message('debug', 'EMAILMANAGER JSON: ' . $encoded_data . "\nHeaders: \n\t" . implode("\n\t", $headers) . "\nReturn:\n$return");
		...
		log_message('debug', 'EMAILMANAGER http code:' . $httpCode);
		
		if (intval($httpCode / 100) != 2) {
			$message = json_decode($return)->Message;
			show_error('Error while mailing. Emailmanager returned HTTP code ' . $httpCode . ' with message "'.$message.'"');
		}
	}
